...
def load_embedding_matrix(embedding, words, embedding_size, save_to_path=None):
    ...
    for i, word in enumerate(words):
        # if word == 'PAD' or word not in model or word == 'UNK':
        ...
        if word == '<pad>':
            continue
        elif word not in embedding or word == '<oov>':
            # if word != 'PAD':
            embedding_matrix[i] = uniform.Uniform(-0.25, 0.25).sample(torch.Size([embedding_size]))
        else:
...
def get_loaders(x_train, x_val, x_test, y_train, y_val, y_test, batch_size, device):
    ...
    val_loader = torch.utils.data.DataLoader(val_array, batch_size, shuffle=True)
