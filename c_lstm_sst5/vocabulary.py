class Voc:
    def __init__(self):
        ...
        self.UNK_token = 1
        self.word2index = {"<pad>": self.PAD_token, "<oov>": self.UNK_token}  # mapping from words to indexes
        self.word2count = {"<pad>": 0}  # count of each word
        self.index2word = {self.PAD_token: "<pad>", self.UNK_token: "<oov>"}  # reverse mapping of indexes to words
    ...
    def indexesFromSentence(self, sentence):
        ...
        for word in sentence.split(' '):
            if word in self.get_words():
                results.append(self.word2index[word])
            else:
                results.append(self.word2index['<oov>'])
        return results
    ...
    def get_words(self, max_vocab_size=None):
